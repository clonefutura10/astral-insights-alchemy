...
const AstrologyConsultation = () => {
  ...
  const processUserMessage = async (userInput: string) => {
    ...
    try {
      // Use fallback response system directly
  ...
  const renderMessageText = (text: string) => {
    // Split by lines first to handle numbered lists properly
    ...
    lines.forEach((line, lineIndex) => {
      ...
      if (numberedListMatch) {
        ...
        elements.push(
          <div key={`numbered-${lineIndex}`} className="ml-4 mb-2 flex">
            <span className="text-blue-300 font-medium mr-2">{number}</span>
            <span className="text-blue-100">{renderInlineFormatting(content)}</span>
          </div>
        );
        return;
      }
      ...
      if (line.startsWith('• ')) {
        elements.push(
          <div key={`bullet-${lineIndex}`} className="ml-4 mb-2 text-blue-100">
      ...
      if (line.startsWith('#')) {
        ...
        const className = level === 1 ? 'text-xl font-bold text-yellow-300 mb-3 mt-4' : 
                         level === 2 ? 'text-lg font-bold text-blue-200 mb-2 mt-3' : 
                         'text-base font-semibold text-blue-300 mb-1 mt-2';
        elements.push(<div key={`header-${lineIndex}`} className={className}>{renderInlineFormatting(text)}</div>);
        return;
      }
      
      // Check for horizontal rule
      if (line === '---') {
        elements.push(<hr key={`hr-${lineIndex}`} className="border-blue-300/30 my-4" />);
        return;
      }
      ...
      if (line.trim()) {
        elements.push(
          <div key={`text-${lineIndex}`} className="mb-1">
